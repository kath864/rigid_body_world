...
class RampScene:
    ...
    def create_table(self, table_d):
        # Setup table
        # could optionally have one already created in the .blend
        # file and simple edit a few details here
        table_d['appearance'] = 'table'
        self.create_block('table', table_d)

    def load_scene(self, scene_dict):
        """ Configures the ramp, table, and balls
        """
        # Setup ramp
        if not self.hide_table:
            self.create_table(scene_dict['table'])
        # Load Objects
    ...
    def _frame_set(self,frame):
        ...
        n_objs = len(positions)
        for obj_i in range(n_objs):
            obj_name = self.obj_names[obj_i]
            obj = bpy.data.objects[obj_name]
            self.move_obj(obj, positions[obj_i])
            self.rotate_obj(obj, rotations[obj_i])
...
def parser(args):
    ...
    p.add_argument('--hide_table', action = 'store_true',
                        help = 'Hide the ramp and table')
...
def main():
    ...
    if args.render_mode == 'default':
        print('Using resolution {}'.format(args.resolution))
